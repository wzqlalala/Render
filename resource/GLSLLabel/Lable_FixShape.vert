...
void main()
{
	...
	mat4 model;
	//将初始应力添加点从模型坐标系转化到通用坐标系下坐标
	vec3 pos_norm = ModelToNormPositon(aPosition,uProjection_m,uView_m,uModel_m);
	Depth = 0.5+pos_norm.z*0.5-0.01;  //输出深度值 和添加点的深度值保持一致

	vec3 pos_c = NormToModelPositon(pos_norm,uProjection_c,uView_c,uModel_c);
	model = uModel_c*translate(pos_c.x,pos_c.y,pos_c.z)*scale(1.3*aSize,1.3*aSize,1.3*aSize);
