...
void main()
{
	...
	vec3 pos_norm = uModelToNormPositon(aPosition,uProjection_m,uView_m,uModel_m);
	Depth = 0.5+pos_norm.z*0.5-0.01;  //输出深度值 让文字的深度值和箭头添加点的深度值保持一致
	vec3 pos_c = NormTouModelPositon(pos_norm,uProjection_c,uView_c,uModel_c);
