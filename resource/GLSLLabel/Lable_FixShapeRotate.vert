...
layout (location = 7) in vec3 aDirection; //指向
...
void main()
{
	...
	mat4 model = uModel_c;
	vec3 iniRotateVec = vec3(0,0,1);
	//旋转对应旋转轴
	float angle = degrees(acos(dot(iniRotateVec,aDirection)/length(iniRotateVec)/length(aDirection)));//获取旋转角度
	if((abs(abs(angle)-180)<0.1))//判断夹角为180
	{
		vec3 axisRotate = vec3(0,1,0);				//获取旋转轴
		model = rotate(axisRotate.x,axisRotate.y,axisRotate.z,180) * model;
	}
	else if(abs(angle)>0.1)//夹角不为0
	{	
		vec3 axisRotate = normalize(cross(iniRotateVec,aDirection));	 
	...
	Depth = 0.5+pos_norm.z*0.5-0.01;  //输出深度值 让深度值和添加点的深度值保持一致
	vec3 pos_c = NormToModelPositon(pos_norm,uProjection_c,uView_c,uModel_c);
	model = translate(pos_c.x,pos_c.y,pos_c.z) *scale(0.6*aSize,0.6*aSize,0.6*aSize)* model;
