...
namespace MViewBasic
{
	...
	mCameraCommonView::mCameraCommonView(QWidget *widget)
		: m_widget(widget),
		m_lastFrame(0.0),
		m_deltaTime(0.0),
		m_timerId(0),
	...
	void mCameraCommonView::handle(QEvent *event)
	{
		switch (event->type()) {
		...
		case QEvent::FocusIn:
		{
			...
			QCursor::setPos(m_widget->mapToGlobal(m_lastMousePos));//鼠标在中�?            
	...
	void mCameraCommonView::setKeyMap(CameraMovement key, Qt::Key value)
	{
		m_keyMap[key] = value;
	}

	float mCameraCommonView::getMoveSpeed() const
	{
		return MovementSpeed;
	}

	void mCameraCommonView::setMoveSpeed(float value)
	{
		MovementSpeed = value;
	}

	float mCameraCommonView::getSensitivity() const
	{
		return MouseSensitivity;
	}

	void mCameraCommonView::setSensitivity(float value)
	{
		MouseSensitivity = value;
	}

	float mCameraCommonView::getYaw() const
	{
		return Yaw ;
	}

	void mCameraCommonView::setYaw(float value)
	{
		Yaw  = value;
		calculateCameraDirection();
	}

	float mCameraCommonView::getPitch() const
	{
		return Pitch ;
	}

	void mCameraCommonView::setPitch(float value)
	{
		Pitch  = value;
		calculateCameraDirection();
	}

	void mCameraCommonView::setPitchConstrained(bool flag)
	{
		m_isPitchConstrained = flag;
	}

	float mCameraCommonView::getFov() const
	{
		return Zoom;
	}

	void mCameraCommonView::setFov(float value)
	{
		Zoom = value;
	}

	QVector3D mCameraCommonView::getCameraPos() const
	{
		return Position ;
	}

	void mCameraCommonView::setCameraPos(const QVector3D &value)
	{
		Position  = value;
	}

	QVector3D mCameraCommonView::getCameraUp() const
	{
		return _Up;
	}

	void mCameraCommonView::setCameraUp(const QVector3D &value)
	{
		_Up = value;
	}

	QVector3D mCameraCommonView::getCameraDirection() const
	{
		return Front;
	}

	void mCameraCommonView::ResetOrthoAndCamera(QVector3D center, float maxRadius)
	{
	...
	void mCameraCommonView::handleMouseMoveEvent(QMouseEvent *event)
	{
	...
	void mCameraCommonView::handleWheelEvent(QWheelEvent *event)
	{
	...
	void mCameraCommonView::handleTimerEvent(QTimerEvent *event)
	{
	...
	//**************************************** calculate method ********************************************
	void mCameraCommonView::calculateCameraDirection()
	{
	...
	void mCameraCommonView::SetPVMValue()
	{
		_view.setToIdentity();
...
namespace MViewBasic
{
	void mCameraCommonView::ProcessMouseMovement(float xoffset, float yoffset, bool constrainPitch)
	{
	...
	void mCameraCommonView::ZoomAtMouse_Bywheel(int posX, int posY, int yoffset, ScalePerspectice scalePerspectice, float sensitivity)
	{
		Zoom -= (float)yoffset;
		if (Zoom < 1.0f)
			Zoom = 1.0f;
