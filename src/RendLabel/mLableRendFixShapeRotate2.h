...
namespace MLableRend
{
	//通用类型图标绘制
	template<class T>
	class mLableRendFixShapeRotate2 : public mLableRendBase<mDrawArray>
	{
	public:
		//可以初始化指定shapeVertex  
		explicit mLableRendFixShapeRotate2(const QHash<QString, T> &data, const QVector<QVector3D> &shapeVertex, const GLenum drawMode = GL_LINES)
			:mLableRendBase<mDrawArray>(9, drawMode), _shapeVertex(shapeVertex), _data(data)
		{
		};
		//可以更新前指定shapeVertex
		explicit mLableRendFixShapeRotate2(const QHash<QString, T> &data, const GLenum drawMode = GL_LINES)
			:mLableRendBase<mDrawArray>(9, drawMode), _data(data)
		{
		};

		virtual ~mLableRendFixShapeRotate2() override {};
		...
		*/
		virtual void updateSingleRend(LableOperateMode mode, const QString &name) override
		{
			...
			QVector3D dir1 = tmp.lableData.getParameter2().first;
			QVector3D dir2 = tmp.lableData.getParameter2().second;
			FixShapeRotateType type = tmp.lableData.getParameter3();
			switch (type)
			{
			...
			if (mode == Append)
			{
				...
				for (int i = 0; i < pos.size(); i++)
				{
					...
					for (int j = 0; j < _shapeVertex.size(); j++)
					{
						...
						//添加特有的数据
						appendSpecificVertex(pos[i], dir1, dir2);
					}
					//更新顶点数量
					_rendObject->_vertexNum += _shapeVertex.size();
				}
				//更新buffer
				initialBuffer_append<float, float, float>(3, 3, 3, _vertex_Pos, _vertex_Dir, _vertex_Dir2, vertexBegin);
			...
			else if (mode == Delete)
			{
				...
				//更新渲染
				initialBuffer_delete<float, float, float>(3, 3, 3, deleteVertexBegin, deleteVertexSize);
		...
		*/
		virtual void updateAllRend(LableOperateMode mode) override
		{
			if (mode == Append)
			{
				...
				//更新buffer
				initialBuffer_append<float, float>(3, 3, 3, _vertex_Pos, _vertex_Dir, _vertex_Dir2, vertexBegin);
				//清空数据
				deleteSpecificVertex();
				deleteCommonVerrtex();
			}
			else if (mode == Delete)
			{
				...
				//更新渲染
				initialBuffer_delete<float, float, float>(3, 3, 3, 0, deleteVertexSize);
				_vertexIndex.clear();
			}
		...
		*/
		...
		inline void appendSpecificVertex(const QVector3D pos, const QVector3D dir, const QVector3D dir2)
		{
			//图标的位置（模型坐标下）
			...
			_vertex_Dir2.push_back(dir2.x());
			_vertex_Dir2.push_back(dir2.y());
			_vertex_Dir2.push_back(dir2.z());
		}
		/*
		* 删除特有的的顶点数据
		*/
		inline void deleteSpecificVertex()
		{
			...
			vector<float>().swap(_vertex_Dir2);
		}
	...
	private:
		...
		vector<float> _vertex_Dir2;  //需要转动的方向 (不一定存在)
