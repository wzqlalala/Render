...
namespace MPostRend
{
	mPostDragRender::mPostDragRender(QString name, std::shared_ptr<mxr::Application> app, std::shared_ptr<mxr::Group> parent)
	{
		_app = app; 
		_parent = parent;
		_drawable = nullptr;
	...
	mPostSphereRender::mPostSphereRender(QString name, std::shared_ptr<Application> app, std::shared_ptr<Group> parent):mPostDragRender(name, app, parent)
	{
	...
	{
		QVector4D centerPos = (pvm * QVector4D(_sphereCenter, 1.0));//求出球心世界坐标的乘以矩阵后的坐标（范围为-1~1）
		...
		if (WorldPos.distanceToPoint(_sphereCenter) < _sphereRadius)
		{
	...
	void mPostSphereRender::updateUniform(std::shared_ptr<MViewBasic::mViewBase> modelView)
	{
		if (_stateSet)
		{
			...
			model.translate(_sphereCenter);
			model.scale(_sphereRadius);
