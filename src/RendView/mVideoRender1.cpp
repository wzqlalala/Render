...
namespace MBaseRend
{
	mVideoRender::mVideoRender(std::shared_ptr<mxr::Application> app, std::shared_ptr<mxr::Group> parent) :mBaseRender(app, parent)
	{
		...
		connect(reader, &FFMpegReader1::OnFrame, this, [&](AVFrame *pframeRGB) {
			int x = reader->pCodecCtx->width;
			int y = reader->pCodecCtx->height;
			if (_texture_y && _texture_u && _texture_v)
			{
	...
	void mVideoRender::initialVideo(QString filename)
	{
		...
		//if (_texture_y && _texture_u && _texture_v)
		//{
		//	_texture_y->SetData(0, 0, 0, x, y, reader->pFrame->data[0]);
		//	_texture_u->SetData(0, 0, 0, x / 2, y / 2, reader->pFrame->data[1]);
		//	_texture_v->SetData(0, 0, 0, x / 2, y / 2, reader->pFrame->data[2]);
		//}
	...
	void mVideoRender::start()
	{
		...
		{
			reader->Open(reader->Url_);
			reader->isOneFrame = false;
			reader->start();
		}
	}
